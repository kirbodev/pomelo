...
export async function buildQuickActionRow(opts: {
	...
	const matchingActions = (config.actions ?? []).filter(
		(a: QuickActionDefinition) => a.triggers.includes(trigger),
	);

	const builtins = ALL_BUILTINS.filter((action) => action !== opts.executedAction);
