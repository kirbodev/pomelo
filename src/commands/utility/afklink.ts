...
import { accounts, afkCalendars, linkedAccounts, users } from "../../db/schema.js";
...
export class UserCommand extends CommandUtils.PomeloCommand {
    public constructor(context: Command.LoaderContext, options: Command.Options) {
        super(context, {
            ...options,
            description: "Link your Google Calendar to Pomelo to automatically set your AFK status when you're busy.",
    ...
    private async execute(
        ...
        const account = await db.select().from(linkedAccounts).where(eq(linkedAccounts.userId, user.id));
        ...
        const buttonInteraction = await i.awaitMessageComponent(
            {
        ...
        if (!buttonInteraction) {
            await this.container.utilities.componentUtils.disableButtons(i instanceof Message ? i : await i.fetch());
            return;
        }
        ...
        const calendarAccount = await db.select().from(accounts).where(eq(accounts.userId, id));

        if (!calendarAccount[0] || !calendarAccount[0].refresh_token) {
            this.error(modalInteraction, this, {
                error: "AuthError",
            });
            return;
        }

        const calendarUser = await db.select().from(users).where(eq(users.id, calendarAccount[0].userId));
        ...
        if (!calendarScopes || !requiredScopes.every((scope) => calendarScopes.includes(scope))) {
            console.log(calendarScopes, requiredScopes);
            this.error(modalInteraction, this, {
                error: "MissingScopes",
            });
            return;
        ...
        await modalInteraction.editReply({
            embeds: [new EmbedUtils.EmbedConstructor().setTitle(t(LanguageKeys.Commands.Utility.Afklink.successTitle)).setDescription(t(LanguageKeys.Commands.Utility.Afklink.successDesc)).setColor(Colors.Success)],
            components: [],
        });

        this.configure(modalInteraction);
    }
    ...
    private async verifyAccount(
        ...
        if (account.length === 0) {
            this.error(interaction, this, {
                error: "GenericError",
            });
            fail = true;
        }

        const calendarUser = await db.select().from(users).where(eq(users.id, account[0].linkCode));
        ...
        const calendarAcc = await db.select().from(accounts).where(eq(accounts.userId, calendarUser[0].id));
    ...
    private async configure(
        ...
        if (!calendarUser || !calendarAcc) return;

        const oauth = new google.auth.OAuth2({
            clientId: process.env.GOOGLE_CLIENT_ID,
            clientSecret: process.env.GOOGLE_CLIENT_SECRET,
            redirect_uris: ["https://kdv.one", "https://pom.kdv.one", "https://pomelo.kdv.one", "https://pom.kdv.one/api/auth/callback/google", "http://localhost:3000/api/auth/callback/google"],
            projectId: "pomelo-463614",
            credentials: {
        ...
        const calendarNames = [...new Set(calendars.map((calendar) => `${calendar.summary}`).filter((name) => name))];

        const menuId = nanoid();

        const selectMenu = await this.createSelectMenu(calendarNames, user, t, menuId);

        const i = await this.reply(
            interaction,
            {
                embeds: [new EmbedUtils.EmbedConstructor().setTitle(t(LanguageKeys.Commands.Utility.Afklink.selectCalendars)).setDescription(t(LanguageKeys.Commands.Utility.Afklink.selectCalendarsDescription)).setColor(Colors.Default)],
        ...
        const selectInteraction = i.createMessageComponentCollector<ComponentType.StringSelect>(
            {
                filter: (i) => i.customId.startsWith(menuId) && i.user.id === user.id,
                time: 60000 * 5,
            }
        ...
        selectInteraction.on("collect", async (i) => {
            await this.handleSelectMenu(i, calendarAcc.userId, i instanceof Message ? i : i.message, calendarNames, menuId, client, calendars);
        });

        selectInteraction.on("end", async () => {
            await this.container.utilities.componentUtils.disableButtons(i instanceof Message ? i : await i.fetch());
        });
    }
    ...
    private async createSelectMenu(
        ...
        const currentCalendarsEntry = await db.select().from(afkCalendars).where(eq(afkCalendars.userId, user.id));
    ...
    private async handleSelectMenu(
        ...
        if (currentCalendarsEntry.length > 0) {
            await db.update(afkCalendars).set({
        ...
        const editedEmbed = new EmbedUtils.EmbedConstructor(replyMessage.embeds[0]!.data as EmbedData);
