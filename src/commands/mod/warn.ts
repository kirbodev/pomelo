...
import type { WarnActionResult, WarnLevel, PunishResult } from "../../lib/moderation/types.js";
...
export class WarnCommand extends CommandUtils.PomeloSubcommand {
  ...
  public override registerApplicationCommands(registry: Command.Registry) {
    ...
    registry.registerChatInputCommand((builder) => {
      applyLocalizedBuilder(
        ...
        .addSubcommand((sub) =>
          applyLocalizedBuilder(sub, LanguageKeys.Commands.Moderation.Warn.commandName, LanguageKeys.Commands.Moderation.Warn.commandDescription)
            .setName("warn")
  ...
  public override async chatInputRun(interaction: Command.ChatInputCommandInteraction) {
    ...
    if (group === "level" && subcommand === "set") {
      ...
      await this.handleWarnResult(interaction, result, member);
    ...
    for (const ta of result.thresholdActions) {
      if (ta.autoExecuted && ta.results) {
        for (const pr of ta.results) {
          lines.push("Level " + String(ta.level.warnCount) + ": " + this.punishmentResultLine(pr, t));
        }
      } else if (ta.error) {
      ...
      : p.punishment.type === "kick"
        ...
        : p.punishment.type === "ban"
          ? p.punishment.duration
            ? LanguageKeys.Commands.Moderation.WarnSettings.Quickstart.punishmentBan
            : LanguageKeys.Commands.Moderation.WarnSettings.Quickstart.punishmentBanPerm
...
private async confirmAndExecuteLevel(
  ...
  const confirmation = new ComponentUtils.ButtonConfirmationConstructor({
    buttons: {
      confirm: {
        text: t(LanguageKeys.Commands.Moderation.WarnSettings.Quickstart.confirmLevelConfirm),
        style: ButtonStyle.Success,
      },
      cancel: {
        text: t(LanguageKeys.Commands.Moderation.WarnSettings.Quickstart.confirmLevelCancel),
  ...
  const sent = await (channel as any).send({
    components: [dialogContainer, confirmation],
    flags: MessageFlags.IsComponentsV2,
  });

  const button = await (sent as any)
  ...
  if (!button) {
    await (channel as any)
      .send(t(LanguageKeys.Commands.Moderation.WarnSettings.Quickstart.confirmLevelTimeout))
      .catch(() => null);
    return [];
  }
  ...
  const moderator =
    target instanceof Message
      ? target.member instanceof GuildMember
        ? target.member
        : null
      : target.member instanceof GuildMember
        ? target.member
