...
export class BanCommand extends CommandUtils.PomeloSubcommand {
  ...
  public override async chatInputRun(interaction: Command.ChatInputCommandInteraction) {
    ...
    if (subcommand === "ban") {
      ...
      const deleteMessageSeconds = deleteChoice ? (parseInt(deleteChoice, 10) as 0 | 86400 | 259200 | 604800) : undefined;

      await this.executeBan(interaction, user, duration, deleteMessageSeconds, reason ?? undefined);
  ...
  public override async messageRun(message: Message, args: Args) {
    ...
    if (sub === "unban") {
      ...
      await this.executeBan(message, user, undefined, undefined, reason ?? undefined);
    }
  }
  ...
  private async executeBan(
    ...
    const result = await modActionService.ban(guild, moderator, targetMember, reason, {
      duration,
      deleteMessageDays,
    });
    ...
    const desc = duration
      ? t(LanguageKeys.Commands.Moderation.Ban.descTemp, { user: user.tag, duration: this.formatMs(duration) })
      : reason
        ? t(LanguageKeys.Commands.Moderation.Ban.descWithReason, { user: user.tag, reason })
        : t(LanguageKeys.Commands.Moderation.Ban.desc, { user: user.tag });

    const dmSentText = result.dmSent
  ...
  private async executeUnban(
    ...
    const result = await modActionService.unban(guild, moderator, userId, reason);
    ...
    const desc = reason
      ? t(LanguageKeys.Commands.Moderation.Unban.descWithReason, { user: userId, reason })
      : t(LanguageKeys.Commands.Moderation.Unban.desc, { user: userId });
